...
class LoginController extends HomeBaseController
{
    ...
    public function doLogin()
    {
        if ($this->request->isPost()) {
            ...
            //file_put_contents('test.txt',json_encode($data),8);
            ...
            if (!cmf_captcha_check($data['captcha'])) {
                return json_encode(array('msg' => lang('CAPTCHA_NOT_RIGHT')));
            ...
            switch ($log) {
                case 0:
                    ...
                    return json_encode(array('msg' => lang('LOGIN_SUCCESS'),'userid'=>cmf_get_current_user_id()));
                    //$this->success(lang('LOGIN_SUCCESS'), $redirect);
                    break;
                case 1:
                    return json_encode(array('msg' => lang('PASSWORD_NOT_RIGHT')));
                    //$this->error(lang('PASSWORD_NOT_RIGHT'));
                    break;
                case 2:
                    return json_encode(array('msg' => '账户不存在'));
                    //$this->error('账户不存在');
                    break;
                case 3:
                    return json_encode(array('msg' => '账号被禁止访问系统'));
                    //$this->error('账号被禁止访问系统');
                    break;
                default :
                    return json_encode(array('msg' => '未受理的请求'));
                    //$this->error('未受理的请求');
            }
        } else {
            return json_encode(array('msg' => "请求错误"));
           // $this->error("请求错误");
        }
    }
