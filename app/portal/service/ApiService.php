...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        if (!empty($categoryIds)) {
            ...
            if (count($categoryIds) == 1) {
                $cids = ApiService::getCateIds($categoryIds[0]);
                $whereCategoryId = function (Query $query) use ($cids) {
                    $query->where('category_post.category_id', 'in', $cids);
                };
            } else {
    ...
    public static function getCateIds($cid)
    {
        $cids = array();
        $cates = Db::name('portal_category') ->where(array('parent_id' => $cid))->field('id')->select()->toArray();
        if(!empty($cates))
        {
